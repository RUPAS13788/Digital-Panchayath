...
$q = "SELECT * FROM quiz where cid=? ORDER BY date DESC";
$sp = mysqli_prepare($con,$q);
mysqli_stmt_bind_param($sp,'i',$cid);
mysqli_stmt_execute($sp);
$result = mysqli_stmt_get_result($sp);
echo  '<div class="panel1"><div class="table-responsive"><table class="table table-striped title1">
...
if($rowcount == 0){
  ...
  <td><b><a href="account.php?cid='.$cid.'&q=quiz&step=2&eid='.$eid.'&n=1&t='.$total.'" class="pull-right btn sub1" style="margin:0px;background:#99cc32"><span class="glyphicon glyphicon-new-window" aria-hidden="true"></span>&nbsp;<span class="title1"><b>Start</b></span></a></b></td></tr>';
}
else
{

$q1 = "SELECT score FROM history where eid=? and email=?";
$sp1 = mysqli_prepare($con,$q1);
mysqli_stmt_bind_param($sp1,'is',$eid,$email);
mysqli_stmt_execute($sp1);
$result1 = mysqli_stmt_get_result($sp1);
// $q1 = "SELECT score FROM history WHERE eid='$eid' AND email='$email'";
// $sp1 = mysqli_prepare($con,$q1);
// $result1 = mysqli_stmt_get_result($sp1);
$score = 0;
...
echo '<tr style="color:#99cc32"><td>'.$c++.'</td><td>'.$title.'&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td><td>'.$total.'</td><td>'.$sahi*$total.'</td><td>'.$time.'&nbsp;min</td>
  <td><b><a href="#" class="pull-right btn sub1" style="margin:0px;background:red"><span  aria-hidden="true"></span>&nbsp;<span class="title1"><b>'.$score.'</b></span></a></b></td></tr>';
}
}
$c=0;
echo '</table></div></div>';
...
echo'<input type="radio" style="margin-left:0px;" name="ans" value="'.$optionid.'">'.$option.'<br /><br />';
}
...
{
  $email = $_SESSION['username'];
  $q = "SELECT * FROM history WHERE email=? and cid=? ORDER BY date DESC";
$sp = mysqli_prepare($con,$q);
mysqli_stmt_bind_param($sp,'si',$email,$cid);
mysqli_stmt_execute($sp);
$result = mysqli_stmt_get_result($sp);
//$q=mysqli_query($con,"SELECT * FROM history WHERE email=? and cid=? ORDER BY date DESC " )or die('Error197');
